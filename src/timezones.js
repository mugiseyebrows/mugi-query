...
var longNames = ["Africa/Abidjan", "Africa/Accra", "Africa/Addis_Ababa",
...
var trs = [...document.querySelector('#tz-abb tbody').querySelectorAll('tr')]
...
function findLongName(name, longNames) {
	...
	var found = longNames.find( n => n.split('/').slice(1).join('').toLowerCase() == name_ ) 
	
	return found
}

function parseOffset(tz) {
	var m = tz.match(/^UTC ([-+])([0-9]+):([0-9]+)$/)
	var offset
	if (m) {
		offset = m[2] * 3600 + m[3] * 60
		if (m[1] === '-') {
			offset = -offset
		}
		return offset;
...
function createTimeZoneData() {
	...
	for(var i=0; i<zones.length; i++) {
		//console.log(i)
		var [code, name, region, tz] = zones[i]
		if (ambiguous.has(code)) {
			continue
		}
		var longName = findLongName(name, longNames)
		var offset = parseOffset(tz)
		if (longName === undefined && offset === undefined) {
			continue
		}

		//if (longName === undefined) {
			res.push([code, name, longName, tz, offset])
		//}
...
var res = createTimeZoneData()

function formatTimeZone(zone) {
	let [code, name, longName, tz, offset] = zone
	return `{"${code}", TimeZone("${code}", "${name}", "${longName || ""}", "${tz}", ${offset})},`
}

function formatTimeZone2(zone) {
	let [code, name, longName, tz, offset] = zone
	return `"${code}",`
}

var lines = res.map(formatTimeZone)
