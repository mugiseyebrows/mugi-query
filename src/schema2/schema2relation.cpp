...
QStringList Schema2Relation::createQueries(const SName &childTable, const QSqlDatabase& db) const {
...
QStringList Schema2Relation::dropQueries(const SName &childTable, const QSqlDatabase& db) const {
    SqlEscaper es(db);
    QString expr = QString("ALTER TABLE %1 DROP CONSTRAINT %2").arg(es.table(childTable)).arg(es.field(mOldName));
    return {expr};
}
...
QStringList Schema2Relation::modifyQueries(const SName &childTable, const QSqlDatabase& db) const {
    return dropQueries(childTable, db) + createQueries(childTable, db);
}
